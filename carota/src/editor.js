...
exports.create = function(element) {
	...
	element.innerHTML =
		...
		'<div class="carotaTextArea" style="overflow: hidden; position: absolute; height: 0;">' +
			'<textarea id="carotaTextArea" autocorrect="off" autocapitalize="off" spellcheck="false" tabindex="0" ' +
	...
	var handleKey = function(key, selecting, ctrlKey, metaKey) {
		...
		switch (key) {
			...
			case 8: // backspace
				...
				if (start === end && start > 0) { // is the selection zero-length? and is the cursor somewhere besides the very start of the document?
					//console.log('lasteditindex from the doc obj:', doc.lastEditedIndex);
					//console.log('lasteditindex for this range:', formatting.lasteditindex);

					var formatting = doc.range(start - 1, start).getFormatting();

					// is the thing we're deleting something from labeled with the currentEditIndex?
					if ((doc.lastEditedIndex === formatting.lasteditindex) || (undefined === formatting.lasteditindex) ) {
						doc.range(start - 1, start).clear(); // just delete it.
					}
					else { // the thing we're deleting is old - we want this to show up as a deletion.
						doc.range(start - 1, start).setFormatting('lasteditindex', doc.lastEditedIndex);
						doc.range(start - 1, start).setFormatting('deleted', true);
					}
	...
	var paint = function() {
		...
		if (!scaleFactor) { // cache the scaleFactor, because it is a hefty thing to get
	...
	doc.paint = paint; // export paint to the outside world so that we can bypass the event loop when we know we need to repaint
	return doc;
};
