...
Range.prototype.setText = function(text) {
    return this.doc.splice(this.start, this.end, text, false);
};

Range.prototype.setTextUsingDefaultStyle = function(text) {
    return this.doc.splice(this.start, this.end, text, true);
};
...
Range.prototype.addToFormatting = function(attribute, value) {
    var range = this;
    if (range.start === range.end) {
        range.doc.modifyInsertFormatting(attribute, value);
    } else {
        var saved = range.save();
        var template = {};
        template[attribute] = [value]; // this line has been mutated from setFormatting above.
        runs.format(saved, template);
        range.setText(saved);
    }
};

Range.prototype.removeFromFormatting = function(attribute, value) {
    var range = this;
    if (range.start === range.end) {
        range.doc.modifyInsertFormatting(attribute, value);
    } else {
        ...
        var template = {};
        template[attribute] = value; // this line has been mutated from setFormatting above.
        runs.formatRemoveComment(saved, template); // removes the value from formatting
        range.setText(saved);
    }
};
